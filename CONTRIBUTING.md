...
1. Create `sources/<id>/`:
   ...
   - `Justfile` — compose the shared `pipelines/source_*.py` steps the source needs. Copy an existing recipe and adjust:
     - http GeoTIFF → `source_download`; ERDDAP → `source_download_erddap --bbox …`; S3 COG tiles → `source_register_remote --bbox …` (streaming refs, no download); zip → `source_download` + `source_unzip`.
